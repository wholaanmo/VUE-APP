...
export default createStore({
	...
	state: {
		...
		expenses: [],
		personalBudgets: [],
		usdExchangeRate: 56.5,
		selectedMonthYear: null,
		viewPageMonthYear:
			new Date().getFullYear() +
			"-" +
			String(new Date().getMonth() + 1).padStart(2, "0"), // For view page
		addExpenseMonthYear:
			new Date().getFullYear() +
			"-" +
			String(new Date().getMonth() + 1).padStart(2, "0"), // For add expense page
		...
		getExpenses: (state) => {
			...
			return state.expenses.filter((expense) => {
				...
				try {
					...
					const expenseMonthYear = `${expenseDate.getFullYear()}-${String(
	...
	mutations: {
		SET_CURRENT_BUDGET(state, budget) {
			const index = state.personalBudgets.findIndex(
				(b) => b.month_year === budget.month_year
			);
			if (index !== -1) {
		...
		ADD_EXPENSE(state, expense) {
			try {
				...
				console.log(
					"Expense successfully added to store:",
					validatedExpense
				);
			} catch (error) {
		...
		UPDATE_EXPENSE(state, updatedExpense) {
			try {
				if (!updatedExpense?.id) {
					console.error(
						"Invalid expense in UPDATE_EXPENSE:",
						updatedExpense
					);
					return;
				}

				const index = state.addExpenses.findIndex(
	...
	actions: {
		...
		async fetchBudgetForMonth({ commit }, monthYear) {
			try {
				...
				const response = await axios.get(
					`/api/personal-budgets/month/${monthYear}`,
					{
						headers: {
		...
		async fetchExchangeRate({ commit }) {
			try {
				...
				if (!phpToUsdRate) {
					console.warn(
						"USD rate not found in response, using fallback"
					);
				}
		...
		async fetchViewExpenses(
			{ commit, state },
			{ monthYear = null, year = null } = {}
		) {
			try {
				const params = {};

				if (monthYear) {
					const budget = await this.dispatch(
						"fetchBudgetForMonth",
						monthYear
					);
					if (budget?.id) {
